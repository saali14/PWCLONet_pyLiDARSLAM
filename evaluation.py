...
class kittiOdomEval():
    ...
    def convert_to_absolute(self, relative_pose, first_pose):
        abs_pose = {}
        last_pose = first_pose
        for key, pose in relative_pose.items():
            abs_pose[key] = pose @ last_pose
            last_pose = abs_pose[key].copy()
        return abs_pose

    def loadPoses(self, file_name, toCameraCoord, relative: bool = True):
        '''
        ...
        for cnt, line in enumerate(s):
            ...
            if relative:
                if cnt == 0:
                    ...
                    P = np.linalg.inv(poses[first_frame]) @ P
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            poses_gt = self.loadPoses(gt_file_name, toCameraCoord=False, relative=False)
            #first_frame = np.min(list(poses_gt.keys()))
            #poses_result = self.convert_to_absolute(poses_result, poses_gt[first_frame].copy())
