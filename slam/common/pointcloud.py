# -- ME -- read and understood 24-03-2023
...
def __voxel_normal_distribution(pointcloud, voxel_hashes, is_sorted: bool = False):
    ...
    if is_sorted:
        ...
        # -- ME -- sort the pointcloud by hash
    ...
    while _idx < n + 1:
        ...
        # -- ME -- voxel_points have the same hash
    ...
    if not is_sorted:
        # -- ME -- to unsort voxel_ids as they were in the pointcloud
        indices = np.argsort(sort_indices)
        voxel_ids = voxel_ids[indices]

    # -- ME -- voxel_sizes contains the size of each group sharing the same hash code
    # -- ME -- max(voxel_ids) = number of unique hash codes
