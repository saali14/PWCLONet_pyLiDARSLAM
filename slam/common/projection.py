...
from pyLiDAR_SLAM.slam.common.utils import check_tensor, assert_debug
...
class Projector(ABC):
    ...
    def build_projection_map(self,
        ...
        # -- ME -- pixels: (B, N, 2) -> each (batch, point) has its pixel values (u, v) [place in the image]
        # these pixel values aren't ordered by their place in the image
        # that's why we use `order`
        pixels = self.project_pointcloud(pointcloud, height=height, width=width, **kwargs)
        r = pointcloud.norm(dim=2) # -- ME -- (B, N)
        pixel_rows = pixels[:, :, 0].round() # -- ME -- (B, N)
        pixel_cols = pixels[:, :, 1].round() # -- ME -- (B, N)
        ...
        order = torch.argsort(r, dim=1, descending=True) # -- ME -- (B, N) -> ordered indexes of norms of C channels
        order = order.reshape(b, n)
        b_idx = b_idx.reshape(b, n)

        # -- ME -- r[b_idx, order]: (B, N) returns ordered `r` for each b (line) in batch 
        mask = r[b_idx, order] > 0.0

        order = order[mask] # -- ME -- order: (-1,) -> columns with valid pixel values
        b_idx = b_idx[mask] # -- ME -- b_idx: (-1,) -> rows with valid pixel values
        proj_row = pixel_rows[b_idx, order].to(torch.int64) # -- ME -- proj_row: (B, N) will contain valid row pixel values ordered by `C` channels norm
        proj_col = pixel_cols[b_idx, order].to(torch.int64)
        destination_image[b_idx, :, proj_row, proj_col] = image_channels[b_idx, order, :] # -- ME -- we put in each (proj_row, proj_col) pixel its value
