...
class KITTIOdometrySequence(Dataset):
    ...
    def correct_scan(scan: np.ndarray):
        ...
        n = scan.shape[0]
        # -- ME -- z.shape = (n, 3)
        # `z` represents the z-axis of each point
        z = np.tile(np.array([[0, 0, 1]], dtype=np.float32), (n, 1))
        # -- ME -- the cross product results in a vector
        # that is perpendicular to the `xyz` and `z`
        # hence axes = [(-yi, xi, 0) for i in range(n)]
        axes = np.cross(xyz, z)
        # Normalize the axes
